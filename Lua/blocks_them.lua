function effectblock()
	...
	for i,unit in ipairs(units) do
		...
		if (unit.className ~= "level") then		
			...
			local isthem = hasfeature(name,"is","them",unit.fixed)
			local isus = hasfeature(name,"is","us",unit.fixed)
			...
			local playercolours = {isyou, isthem, isus}
			local cplayercolours = {{4,1}, {3,2}, {3,1}}
			...
			-------------------------------------------------------
				...
				for a=1,#cplayercolours do
					if (playercolours[a] ~= nil) then
						local c = cplayercolours[a]
...
function block(small_)
	...
	isyou4 = getunitswitheffect("them",false,delthese)
	isyou5 = getunitswitheffect("us",false,delthese)
					...
					if (unitmap[tileid] == nil) or (#unitmap[tileid] == 0) then
						...
						for a,rules in ipairs(emptythings) do
							...
							if (rule[2] == "is") then
								...
								elseif ((rule[3] == "you") or (rule[3] == "you2") or (rule[3] == "3d") or (rule[3] == "them") or (rule[3] == "us")) and testcond(conds,2,i,j) then
									candefeat = true
									canwin = true
								...
								if canbonus and ((hasfeature("level","is","you",1,i,j) ~= nil) or (hasfeature("level","is","you2",1,i,j) ~= nil) or (hasfeature("level","is","3d",1,i,j) ~= nil) or (hasfeature("level","is","them",1,i,j) ~= nil) or (hasfeature("level","is","us",1,i,j) ~= nil)) and floating_level(2,i,j) then
									bonus = true
								end
								
								if canend and ((hasfeature("level","is","you",1,i,j) ~= nil) or (hasfeature("level","is","you2",1,i,j) ~= nil) or (hasfeature("level","is","3d",1,i,j) ~= nil) or (hasfeature("level","is","them",1,i,j) ~= nil) or (hasfeature("level","is","us",1,i,j) ~= nil)) and floating_level(2,i,j) then
									ending = true
								end
		...
		if (#things > 0) then
			for i,rules in ipairs(things) do
				...
				if (rule[1] == "level") and (rule[2] == "is") and testcond(conds,1) then
					...
					if (action == "you") or (action == "you2") or (action == "3d") or (action == "them") or (action == "us")  then
					...
					elseif (action == "defeat") then
						...
						local yous4 = findfeature(nil,"is","them")
						local yous5 = findfeature(nil,"is","us")
					...
					elseif (action == "bonus") then
						...
						if ((#findallfeature("empty","is","you") > 0) or (#findallfeature("empty","is","you2") > 0) or (#findallfeature("empty","is","3d") > 0) or (#findallfeature("empty","is","them") > 0) or (#findallfeature("empty","is","us") > 0)) and floating_level(2) then
							bonusget = true
					...
					elseif (action == "win") then
						...
						if (hasfeature("level","is","you",1) ~= nil) or (hasfeature("level","is","you2",1) ~= nil) or (hasfeature("level","is","3d",1) ~= nil) or (hasfeature("level","is","them",1) ~= nil) or (hasfeature("level","is","us",1) ~= nil) or emptyyou then
							canwin = true
						end
					...
					if (rule[1] == "image") and (rule[2] == "is") and (#conds == 0) then
						...
						if (nums[num] ~= nil) then
							...
							if (data[2] ~= "win") then
								...
								local yous4 = findallfeature(nil,"is","them",true)
								local yous5 = findallfeature(nil,"is","us",true)
...
function findplayer(undoing)
	...
	local players4 = findfeature(nil,"is","them")
	local players5 = findfeature(nil,"is","us")
