function movecommand(ox,oy,dir_,playerid_,dir_2,no3d_)
	--[[ 
		@mods(co-op) - Override reason - Must override you2 for non 3d mode
	 ]]
	...
	if (playerid == 1) then
		...
		levelmove2 = findfeature("level","is","us")
	...
	elseif (playerid == 2) then
		levelmove = findfeature("level","is","them")
		levelmove2 = findfeature("level","is","us")
		...
		if (finaltake == false) then
			if (take == 1) then
				...
				if (playerid == 1) then
					...
					players2,empty2 = findallfeature(nil,"is","us")
				...
				elseif (playerid == 2) then
					players,empty = findallfeature(nil,"is","them")
					players2,empty2 = findallfeature(nil,"is","us")
			...
			elseif (take == 8) then
				--[[]
				local haunts = findallfeature(nil,"haunt",nil)
